...
describe ConsumableDecorator do
  ...
  describe '.display_amount_until_reorder' do
    ...
    context 'There are more than the minimum amount' do
      let(:consumable) { FactoryGirl.create(:consumable, stock_level: 100, min_stock_level: 10) }
    ...
    context 'There are less than the minimum amount' do
      let(:consumable) { FactoryGirl.create(:consumable, stock_level: 90 , min_stock_level: 100) }
    ...
    context 'There are exactly the minimum amount' do
      let(:consumable) { FactoryGirl.create(:consumable, stock_level: 100 , min_stock_level: 100) }
